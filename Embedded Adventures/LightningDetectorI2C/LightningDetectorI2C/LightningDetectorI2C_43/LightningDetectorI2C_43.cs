...
namespace Gadgeteer.Modules.EmbeddedAdventures
{
    /// <summary>
    ...
    /// </summary>
    public class LightningDetectorI2C : Module
    {
        /// <summary>Represents the delegate used for the Disturbance Detected event.</summary>
        /// <param name="sender">The object that raised the event.</param>
        public delegate void DisturbanceEventHandler(LightningDetectorI2C sender);

        /// <summary>Represents the delegate used for the Lightning Detected event.</summary>
        /// <param name="sender">The object that raised the event.</param>
        /// <param name="e">The event arguments.</param>
        public delegate void LightningEventHandler(LightningDetectorI2C sender, LightningEventArgs e);

        /// <summary>Represents the delegate used for the Noise Detected event.</summary>
        /// <param name="sender">The object that raised the event.</param>
        public delegate void NoiseEventHandler(LightningDetectorI2C sender);
        ...
        private const string lightningDetectorFailedToPerformI2cTransaction =
            "Lightning Detector Failed to perform I2C transaction";

        private const byte TuneCapSetting = 3; //my test say 4 their bag said 3
        ...
        public LightningDetectorI2C(int socketNumber)
        {
            ...
            var con = new I2CDevice.Configuration(0x03, 400);

            i2c = new I2CDevice(con);
            //GTI.I2CBusFactory.Create(socket, 0x03, 400, this);
        ...
        {
            Reset(); //this will turn the power off????
        ...
        public bool Calibrate()
        {
            ...
            //Calibration command
            if (
                i2c.Execute(new I2CDevice.I2CTransaction[] {I2CDevice.CreateWriteTransaction(new byte[] {0x3D, 0x96})},
                    1000) == 0)
            {
                ErrorPrint(lightningDetectorFailedToPerformI2cTransaction);
            }
        ...
        private void OnLightningEvent(LightningDetectorI2C sender, LightningEventArgs e)
        {
        ...
        private void OnDisturbanceEvent(LightningDetectorI2C sender)
        {
        ...
        private void OnNoiseEvent(LightningDetectorI2C sender)
        {
        ...
        {
            var x = i2c.Execute(new I2CDevice.I2CTransaction[]
            {
                I2CDevice.CreateWriteTransaction(new[] {register}),
                I2CDevice.CreateReadTransaction(read2)
            }, 1000);
            ...
            try
            {
                if (
                    i2c.Execute(
                        ...
                        {I2CDevice.CreateWriteTransaction(new[] {(byte) register, regval})}, 1000) == 0)
                {
                    ErrorPrint(lightningDetectorFailedToPerformI2cTransaction);
                }
            }
        ...
        {
            var x =
                i2c.Execute(new I2CDevice.I2CTransaction[] {I2CDevice.CreateWriteTransaction(new byte[] {0x3C, 0x96})},
                    1000); //reset command
            ...
            //Debug.Print("After Reset Mode down mode is " +
            //            ((RegisterRead(Register.PWD, BitMask.PWD) == (byte)Mode.Enabled)
            //                ? "enabled"
            //                : "disabled"));
        }
